    ...
    <q-card v-if="user" class="my-card col-12 col-md-10 row justify-center br">
      <q-form class="col-12 ">
        <q-card-section class="full-width bg-accent">
          <div class="text-h6 full-width text-center text-white">chi tiết thông tin </div>
        </q-card-section>
        <q-card-section class="row items-center text-center">
          <div class="col-3 col-md-4">id: {{ user.id }}</div>
          <q-input class="col-9 col-md-8" v-model="user.name" type="text" label-color="blue" label=" họ tên"  />
          <div class="col-12">
            <hr />
          </div>
          <q-input
            ...
            type="text"
            label=" địa chỉ " label-color="blue"
          />
          ...
          <q-input
            ...
            type="text"
            label=" số điện thoại" label-color="blue"
          />
        ...
        <q-card-actions  align="right">
          <q-btn class="bg-primary text-white"  label="thay đổi" @click="updateAccount"/>
        </q-card-actions>
        ...
        <q-card-section class="row items-center justify-center">
          <q-input class="col-7" v-model="user.email" type="text" label=" tên đăng nhập"  />
          <div class="col-12"><br></div>
          <q-input v-model="password" type="text" label="mật khẩu cũ" />
          <q-input v-model="newpass" :rules="[(val)=>val?.length > 8 || 'không đủ chiều dài'] " type="text" label=" mật khẫu mới" />
          <q-input v-model="reNewpass" :rules="[(val)=>val== newpass || 'không khớp']" type="text" label=" mật khẫu mới" />
        </q-card-section>
        <q-card-actions  align="right">
          <q-btn class="bg-primary text-white" label=" thay đổi tài khoảng" @click="updateUP" />
        </q-card-actions>
        <q-card-section>
          <hr>
        </q-card-section>
        <q-card-actions  align="center">
          <q-btn color="warning"  label=" đăng xuất tất cả thiết bị"  @click="logoutAllDevice()"/>
        </q-card-actions>
      </q-form>
    </q-card>
  </q-page>
